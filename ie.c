...
void execution_stage(inst_t instructions[], struct mips_status *mips_status_t,int32_t registers[])
{
    ...
    case (BZ):
        if ((registers[current_int.rs_i_type]) == 0)
        {
            mips_status_t->zero_flag = true;
            //printf("Conditional BZ  Branch is taken\n");
            mips_status_t->pc = (current_int.imm_i_type + mips_status_t->pc);
    ...
    case (BEQ):
        if ((registers[current_int.rt_i_type]) == (registers[current_int.rs_i_type]))
        {
            mips_status_t->zero_flag = true;
